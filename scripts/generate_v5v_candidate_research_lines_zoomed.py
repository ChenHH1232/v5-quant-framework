...
def main() -> None:
    ...
    plotted = {label: (normalize(values, days), color, width, status) for label, (values, color, width, status) in source.items()}
    candidate_values = dict(plotted["主候选 internal_subsleeve_mom12_70_30"][0])
    relative = {label: ([(day, value / candidate_values[day] * 100) for day, value in values], color, width, status) for label, (values, color, width, status) in plotted.items()}

    full_values = [value for values, _, _, _ in plotted.values() for _, value in values]
    full_lower, full_upper = min(90, math.floor(min(full_values) / 10) * 10), math.ceil(max(full_values) / 10) * 10
    relative_values = [value for values, _, _, _ in relative.values() for _, value in values]
    relative_lower = math.floor((min(relative_values) - 0.5))
    relative_upper = math.ceil((max(relative_values) + 0.5))

    image = Image.new("RGB", (3200, 2200), "#ffffff")
    ...
    text(draw, (82, 56), "V5 同窗口研究路径：完整净值轨迹与相对主候选的放大差距", navy, 42, True)
    text(draw, (84, 123), "全部使用 2021-05-06 至 2026-05-29 的 1,228 个交易日；上图保留完整路径，下图以主候选 = 100 放大显示分支差异。", muted, 22)
    text(draw, (84, 163), "正式表现正文仅允许 repaired baseline 与主候选 pairwise comparison；其余线仅为研究/执行观察，不构成收益排名或升级依据。", "#9a5b13", 21, True)

    left, right = 165, 2980
    top1, bottom1 = 310, 1045
    top2, bottom2 = 1165, 1450
    start_ordinal, end_ordinal = ordinal(START), ordinal(END)

    def draw_panel(top: int, bottom: int, lower: float, upper: float, tick_step: int, title: str, unit: str, years: bool) -> None:
        ...
        for value in range(int(lower), int(upper) + 1, tick_step):
            ...
            text(draw, (72, y - 13), str(value), muted, 19)
        text(draw, (70, top + 48), unit, muted, 17, True)
        text(draw, (left + 22, top + 18), title, "#30735f", 19, True)

    draw_panel(top1, bottom1, full_lower, full_upper, 20, "上图：完整归一化净值路径（起点 = 100）", "归一化\n净值指数", False)
    draw_panel(top2, bottom2, relative_lower, relative_upper, 2, "下图：相对主候选净值指数（主候选恒为 100；用于放大分支差异）", "相对主候选\n净值指数", True)

    def points(values: list[tuple[str, float]], top: int, bottom: int, lower: float, upper: float) -> list[tuple[float, float]]:
    ...
    def draw_lines(lines: dict[str, tuple[list[tuple[str, float]], str, int, str]], top: int, bottom: int, lower: float, upper: float) -> None:
        for label, (values, color, width, status) in lines.items():
            if status == "observation":
                draw.line(points(values, top, bottom, lower, upper), fill="#ffffff", width=width + 7, joint="curve")
                draw.line(points(values, top, bottom, lower, upper), fill=color, width=width, joint="curve")
        values, color, width, _ = lines["repaired baseline"]
        draw.line(points(values, top, bottom, lower, upper), fill="#ffffff", width=width + 7, joint="curve")
        draw.line(points(values, top, bottom, lower, upper), fill=color, width=width, joint="curve")
        values, color, width, _ = lines[candidate_label]
        draw.line(points(values, top, bottom, lower, upper), fill="#ffffff", width=width + 9, joint="curve")
        draw.line(points(values, top, bottom, lower, upper), fill=color, width=width, joint="curve")

    draw_lines(plotted, top1, bottom1, full_lower, full_upper)
    draw_lines(relative, top2, bottom2, relative_lower, relative_upper)

    legend_top = 1570
    draw.rounded_rectangle((165, legend_top, 1585, 1902), radius=10, fill="#fbfdff", outline="#c8d6e5", width=2)
    ...
    panel_left = 1660
    draw.rounded_rectangle((panel_left, legend_top, 2980, 1902), radius=10, fill="#fffaf5", outline="#edc985", width=2)
    ...
    text(draw, (panel_left + 38, legend_top + 260), "相对图只放大差异，不改变原始净值或治理结论；不得由图形高度推导 accepted、可部署或更优的正式结论。", muted, 18)
    text(draw, (165, 2030), "数据来源：冻结的 JoinQuant 日度净值、V5g limited engineering 与 V5h execution backtest 输出；没有生成新 target、没有改变模型状态。", muted, 18)
